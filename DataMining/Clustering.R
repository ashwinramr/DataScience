...
## Let us first import the data - Cust_Spend_Data.csv

RCDF <- read.csv(file.choose())
View(RCDF)
...
clus1 <- hclust(d.euc, method = "ward.D2")
...
d.euc
clus2 <- hclust(d.euc, method = "average") # Creates a dendrogram
clus2
#View the dendrogram visually
plot(clus2, labels = as.character(RCDF[,2])) 
# Outline the clusters
rect.hclust(clus2, k=3, border="blue")
clus2$height
